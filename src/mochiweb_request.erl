...
%% @spec new(Transport, Socket, Method, RawPath, Version, headers()) -> request()
%% @doc Create a new request instance.
new(Transport, Socket, Method, RawPath, Version, Headers) ->
    {?MODULE, [Transport, Socket, Method, RawPath, Version, Headers]}.

%% @spec get_header_value(K, request()) -> undefined | Value
%% @doc Get the value of a given request header.
get_header_value(K, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, Headers]}) ->
    mochiweb_headers:get_value(K, Headers).

get_primary_header_value(K, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, Headers]}) ->
    mochiweb_headers:get_primary_value(K, Headers).

get_combined_header_value(K, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, Headers]}) ->
...
get(transport, {?MODULE, [Transport, _Socket, _Method, _RawPath, _Version, _Headers]}) ->
    Transport;
get(socket, {?MODULE, [_Transport, Socket, _Method, _RawPath, _Version, _Headers]}) ->
    Socket;
get(scheme, {?MODULE, [Transport, Socket, _Method, _RawPath, _Version, _Headers]}) ->
    case Transport:type(Socket) of
        tcp -> http;
        ssl -> https
    end;
get(method, {?MODULE, [_Transport,_Socket, Method, _RawPath, _Version, _Headers]}) ->
    Method;
get(raw_path, {?MODULE, [_Transport, _Socket, _Method, RawPath, _Version, _Headers]}) ->
    RawPath;
...
get(headers, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, Headers]}) ->
    Headers;
get(peername, {?MODULE, [Transport, Socket, _Method, _RawPath, _Version, _Headers]}) ->
...
get(path, {?MODULE, [_Transport, _Socket, _Method, RawPath, _Version, _Headers]}) ->
...
get(body_length, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
...
get(range, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
...
dump({?MODULE, [_Transport, _Socket, Method, RawPath, Version, Headers]}) ->
...
send(Data, {?MODULE, [Transport, Socket, _Method, _RawPath, _Version, _Headers]}) ->
...
%%      idle timeout.
recv(Length, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
    recv(Length, ?IDLE_TIMEOUT, THIS).

%% @spec recv(integer(), integer(), request()) -> binary()
%% @doc Receive Length bytes from the client as a binary, with the given
%%      Timeout in msec.
recv(Length, Timeout, {?MODULE, [Transport, Socket, _Method, _RawPath, _Version, _Headers]}) ->
...
body_length({?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
...
recv_body({?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
    recv_body(?MAX_RECV_BODY, THIS).

%% @spec recv_body(integer(), request()) -> binary()
%% @doc Receive the body of the HTTP request (defined by Content-Length).
%%      Will receive up to MaxBody bytes.
recv_body(MaxBody, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
...
stream_body(MaxChunkSize, ChunkFun, FunState, {?MODULE,[_Transport, _Socket,_Method,_RawPath,_Version,_Headers]}=THIS) ->
    stream_body(MaxChunkSize, ChunkFun, FunState, undefined, THIS).

stream_body(MaxChunkSize, ChunkFun, FunState, MaxBodyLength,
            {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
...
start_response({Code, ResponseHeaders}, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
    start_raw_response({Code, ResponseHeaders}, THIS).

%% @spec start_raw_response({integer(), headers()}, request()) -> response()
%% @doc Start the HTTP response by sending the Code HTTP response and
%%      ResponseHeaders.
start_raw_response({Code, ResponseHeaders}, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
...
%%      will set header defaults such as Server
%%      and Date if not present in ResponseHeaders.
format_response_header({Code, ResponseHeaders}, {?MODULE, [_Transport, _Socket, _Method, _RawPath, Version, _Headers]}=THIS) ->
    HResponse = mochiweb_headers:make(ResponseHeaders),
...
respond({Code, ResponseHeaders, {file, IoDevice}},
        {?MODULE, [_Transport, _Socket, Method, _RawPath, _Version, _Headers]}=THIS) ->
    ...
    Response;
respond({Code, ResponseHeaders, chunked}, {?MODULE, [_Transport, _Socket, Method, _RawPath, Version, _Headers]}=THIS) ->
    HResponse = mochiweb_headers:make(ResponseHeaders),
    ...
    start_response({Code, HResponse1}, THIS);
respond({Code, ResponseHeaders, Body}, {?MODULE, [_Transport, _Socket, Method, _RawPath, _Version, _Headers]}=THIS) ->
...
not_found({?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
    not_found([], THIS).

%% @spec not_found(ExtraHeaders, request()) -> response()
%% @doc Alias for <code>respond({404, [{"Content-Type", "text/plain"}
%% | ExtraHeaders], &lt;&lt;"Not found."&gt;&gt;})</code>.
not_found(ExtraHeaders, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
...
ok({ContentType, Body}, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
    ok({ContentType, [], Body}, THIS);
ok({ContentType, ResponseHeaders, Body}, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
    HResponse = mochiweb_headers:make(ResponseHeaders),
...
should_close({?MODULE, [_Transport, _Socket, _Method, _RawPath, Version, _Headers]}=THIS) ->
...
cleanup({?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}) ->
...
parse_qs({?MODULE, [_Transport, _Socket, _Method, RawPath, _Version, _Headers]}) ->
...
get_cookie_value(Key, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
    proplists:get_value(Key, parse_cookie(THIS)).

%% @spec parse_cookie(request()) -> [{Key::string(), Value::string()}]
%% @doc Parse the cookie header.
parse_cookie({?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
...
parse_post({?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
...
stream_unchunked_body(0, Fun, FunState, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}) ->
    Fun({0, <<>>}, FunState);
stream_unchunked_body(Length, Fun, FunState,
                      {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) when Length > 0 ->
...
read_chunk_length({?MODULE, [Transport, Socket, _Method, _RawPath, _Version, _Headers]}) ->
    ok = Transport:setopts(Socket, [{packet, line}]),
...
read_chunk(0, {?MODULE, [Transport, Socket, _Method, _RawPath, _Version, _Headers]}) ->
    ok = Transport:setopts(Socket, [{packet, line}]),
    F = fun (F1, Acc) ->
                case Transport:recv(Socket, 0, ?IDLE_TIMEOUT) of
                    {ok, <<"\r\n">>} ->
    ...
    put(?SAVE_RECV, true),
    Footers;
read_chunk(Length, {?MODULE, [Transport, Socket, _Method, _RawPath, _Version, _Headers]}) ->
...
read_sub_chunks(Length, MaxChunkSize, Fun, FunState,
                {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) when Length > MaxChunkSize ->
...
serve_file(Path, DocRoot, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
    serve_file(Path, DocRoot, [], THIS).

%% @spec serve_file(Path, DocRoot, ExtraHeaders, request()) -> Response
%% @doc Serve a file relative to DocRoot.
serve_file(Path, DocRoot, ExtraHeaders, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
...
maybe_redirect([], FullPath, ExtraHeaders, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
    maybe_serve_file(directory_index(FullPath), ExtraHeaders, THIS);

maybe_redirect(RelPath, FullPath, ExtraHeaders,
               {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, Headers]}=THIS) ->
...
maybe_serve_file(File, ExtraHeaders, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
...
%%
accepted_encodings(SupportedEncodings, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
...
%%
accepts_content_type(ContentType1, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
...
%%             ["text/html", "application/json"]
%%
accepted_content_types(Types1, {?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
    Types = lists:map(
...
accept_header({?MODULE, [_Transport, _Socket, _Method, _RawPath, _Version, _Headers]}=THIS) ->
